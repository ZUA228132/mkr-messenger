...
class EditTransportViewController: UITableViewController {
    ...
    private func login(emailAddress: String) {
        ...
        // Use MKR defaults for @mkr.su addresses
        let mkrDefaults = MKRConfig.getDefaultLoginParams(for: emailAddress)

        loginParam.imapServer = imapServerCell.getText() ?? (mkrDefaults.imapServer.isEmpty ? nil : mkrDefaults.imapServer)
        loginParam.imapPort = imapPortCell.getText().flatMap { Int($0) } ?? mkrDefaults.imapPort
        loginParam.imapUser = imapUserCell.getText()
        loginParam.imapSecurity = imapSecurityValue.value
        loginParam.smtpServer = smtpServerCell.getText() ?? (mkrDefaults.smtpServer.isEmpty ? nil : mkrDefaults.smtpServer)
        loginParam.smtpPort = smtpPortCell.getText().flatMap { Int($0) } ?? mkrDefaults.smtpPort
